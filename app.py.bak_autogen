...
app = Flask("MedLife")
app.config["SQLALCHEMY_DATABASE_URI"] = 'mysql://root:@localhost/medlife2'
db = SQLAlchemy(app)
app.secret_key = "medlife.com"
...
class DoctorInfo(db.Model):
    ...
    def __init__(self, Name, speciality, Gender, Experience):
        self.Name = Name
...
def Diagnosis():
    ...
    return render_template('diagnosis.html', email = session["email"],user_type=session['user_type'])
...
def Diagnosis_Search():
    ...
    # Get a list of doctor IDs for which the user already has appointments
    user_appointments = [appointment.doctor_id for appointment in Appointment.query.filter_by(user_id=user_id).all()]

    # Render the search results in the search.html template
    return render_template('search.html', search_results=search_results, user_appointments=user_appointments, email=session["email"], user_type=session['user_type'])
...
def search_products():
    ...
    products = Product_info.query.filter(
        or_(
            Product_info.Mption.like(f"%{search_query}%"),
            Product_info.Price.lanufacturer.like(f"%{search_query}%"),
            ...
            Product_info.Descriike(f"%{search_query}%")
        )
    ).all()
# if "google_id" in session:
    if session.get('email'):
